<header class="bg-midnight p-6">
    <div class="w-full max-w-6xl py-6 lg:py-0 mx-auto flex flex-col items-center lg:flex-row lg:justify-between lg:items-center text-eggshell">
    <h1 class="mb-8 lg:mb-0"><img src="/missiontripbbq.png" alt="Mission Trip BBQ" class="w-auto h-24"></h1>
    <div class="links flex flex-col lg:block text-center space-y-3 lg:space-x-4">

         <a href="#howitworks" class="text-xl hover:underline">How It Works</a>
        <a href="#food" class="text-xl hover:underline">Our Food</a>
        <a href="#about"class="text-xl hover:underline">About Us</a>
        <a href="#donate"class="text-xl hover:underline">Donate</a>
        <a href="/order" class="text-xl hover:underline bg-blaze text-eggshell font-bold px-4 py-2 rounded-sm">Order BBQ</a>
    </div>
    </div>
    </header>